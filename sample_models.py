...
def cnn_rnn_model(input_dim, filters, kernel_size, conv_stride,
    ...
    # DONE: Add batch normalization
    bn_rnn = BatchNormalization(name='bnn_rnn')(simp_rnn)
    # DONE: Add a TimeDistributed(Dense(output_dim)) layer
    time_dense = TimeDistributed(Dense(output_dim), name='dense')(bn_rnn)
    # Add softmax activation layer
    y_pred = Activation('softmax', name='softmax')(time_dense)
...
def deep_rnn_model(input_dim, units, recur_layers, output_dim=29):
    ...
    for i in range(recur_layers):
        simp_rnn = GRU(units, activation='relu',
            return_sequences=True, implementation=2, name=f'rnn_{i+1}')(x)
        x = BatchNormalization(name=f'bn_rnn_{i+1}')(simp_rnn)
    
    # DONE: Add a TimeDistributed(Dense(output_dim)) layer
    time_dense = TimeDistributed(Dense(output_dim), name='dense')(x)
    # Add softmax activation layer
    y_pred = Activation('softmax', name='softmax')(time_dense)
    # Specify the model
    model = Model(inputs=input_data, outputs=y_pred)
...
def custom_model(input_dim, filters, kernel_size, conv_stride,
    conv_border_mode, units, output_dim=29):
    """ Build a recurrent + convolutional network for speech 
    """
    # Main acoustic input
    ...
    rnn = Bidirectional(GRU(units, activation='relu',
        return_sequences=True, implementation=2),
                            merge_mode='concat', name='rnn')(bn_cnn)
    # DONE: Add batch normalization
    bn_rnn = BatchNormalization(name='bnn_rnn')(rnn)
    # DONE: Add a TimeDistributed(Dense(output_dim)) layer
    time_dense = TimeDistributed(Dense(output_dim), name='dense')(bn_rnn)
    # Add softmax activation layer
    y_pred = Activation('softmax', name='softmax')(time_dense)
...
def final_model(input_dim, filters, kernel_size, conv_stride,
    ...
    x = Dropout(0.2, name='input_dropout')(input_data)
    ...
    gru_rnn = Bidirectional(GRU(units, 
                                activation='relu',
                                dropout=0.3,
                                return_sequences=True, 
                                implementation=2),
                            merge_mode='concat', name='gru_rnn')(bn_cnn)
    bn_rnn = BatchNormalization(name='bnn_rnn')(gru_rnn)
